...
SELECT w.id_pracownika FROM ksiegowosc.wynagrodzenie w -- b)
JOIN ksiegowosc.pracownicy p ON w.id_pracownika = p.id_pracownika
JOIN ksiegowosc.pensje ON pensje.id_pensji = w.id_pensji
WHERE kwota > 1000;
...
SELECT imie, nazwisko, kwota FROM ksiegowosc.pracownicy p -- g)
JOIN ksiegowosc.wynagrodzenie w ON p.id_pracownika = w.id_pracownika
JOIN ksiegowosc.pensje m ON w.id_pensji = m.id_pensji
...
SELECT kwota, imie, nazwisko FROM ksiegowosc.pracownicy p -- i)
JOIN ksiegowosc.wynagrodzenie w ON p.id_pracownika = w.id_pracownika
JOIN ksiegowosc.pensje m ON w.id_pensji = m.id_pensji
ORDER BY kwota, imie, nazwisko;

SELECT imie, nazwisko, z.kwota AS pensja, x.kwota AS premia FROM ksiegowosc.pracownicy p -- j)
JOIN ksiegowosc.wynagrodzenie w ON p.id_pracownika = w.id_pracownika
JOIN ksiegowosc.pensje z ON w.id_pensji = z.id_pensji
JOIN ksiegowosc.premie x ON w.id_premii = x.id_premii
ORDER BY z.kwota DESC, x.kwota DESC;

SELECT stanowisko, COUNT(*) -- k)
FROM ksiegowosc.pensje 
GROUP BY stanowisko;

SELECT stanowisko, kwota, AVG(kwota) AS œr_p³aca, MIN(kwota) AS min_p³aca, MAX(kwota) AS max_p³aca -- l)
FROM ksiegowosc.pensje p
WHERE stanowisko = 'kierownik'
GROUP BY stanowisko, kwota;

SELECT SUM(pensja) AS suma FROM ( -- m)
SELECT SUM(p.kwota) AS pensja
FROM ksiegowosc.pensje p
JOIN ksiegowosc.wynagrodzenie w ON p.id_pensji = w.id_pensji
GROUP BY p.id_pensji)
AS SUMA;

SELECT SUM(p.kwota) AS pensja, p.stanowisko  -- n)
FROM ksiegowosc.pensje p
JOIN ksiegowosc.wynagrodzenie w ON p.id_pensji = w.id_pensji
GROUP BY p.stanowisko;

SELECT stanowisko, COUNT(x.kwota) AS liczba_premii -- o)
FROM ksiegowosc.pensje p
JOIN ksiegowosc.wynagrodzenie w ON p.id_pensji = w.id_pensji
JOIN ksiegowosc.premie x ON w.id_premii = x.id_premii
GROUP BY stanowisko;
